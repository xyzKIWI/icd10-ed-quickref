...
- 外傷碼未明示照護階段時，依本站急診工作流程預設為第七碼 `A`（初期照護）；若為後續照護、後遺症或其他適用情形，請改選正確第七碼。
- 搜尋結果直接點 ICD-10 診斷碼即可複製，不另設「複製代碼」連結；「去小數點」開啟時，剪貼簿內容會移除小數點。
- 外傷小人的多部位模式會顯示各部位完整建議碼；可逐一點代碼複製，或按「展開候選碼」進一步核對。
...
- 未明示照護階段時會依急診工作流程預設第七碼 `A`；這只是操作預設，送出前仍須依實際診療階段確認或改選。
- 多部位模式可直接複製各列建議碼，但側別、骨折型態及第七碼仍須逐項核對，不把一個推論套用到所有項目。
